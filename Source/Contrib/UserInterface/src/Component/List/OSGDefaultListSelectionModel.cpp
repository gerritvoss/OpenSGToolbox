...
void 	DefaultListSelectionModel::addSelectionInterval(UInt32 index0, UInt32 index1)
{
	...
	if (index0 >= 0 && index1 >= 0)
	{
        ...
        ListSelectionEventUnrecPtr TheListSelectionEvent = ListSelectionEvent::create(NULL, 0, 0, 0, false);
		switch (_SelectionMode)
		{
			case SINGLE_SELECTION:
				...
				{   // if it's empty, then you only have to push onto the list
				    ...
				    TheListSelectionEvent = ListSelectionEvent::create(NULL, getSystemTime(), range.StartIndex, range.EndIndex, _ValueIsAdjusting);
				}
				else if (_RangeSelectionList.front() != range) // if the new range is the same as the first one, then nothing has to be done
				{
					...
					_RangeSelectionList.push_back(range);
                    ShouldProduceSelectionChangeEvent = true;
				    TheListSelectionEvent = ListSelectionEvent::create(NULL, getSystemTime(), minMax.StartIndex, minMax.EndIndex, _ValueIsAdjusting);
				}
				break;
			case SINGLE_INTERVAL_SELECTION:
				...
				if (_RangeSelectionList.empty())
				{
                    ...
                    ShouldProduceSelectionChangeEvent = true;
				    TheListSelectionEvent = ListSelectionEvent::create(NULL, getSystemTime(), newRange.StartIndex, newRange.EndIndex, _ValueIsAdjusting);
				}
...
void 	DefaultListSelectionModel::clearSelection(void)
{
    ...
    produceSelectionChanged(ListSelectionEvent::create(NULL, getSystemTime(), _MinSelectionIndex, _MaxSelectionIndex, _ValueIsAdjusting));
}
   ...
   //TODO:Implement
	...
	switch (_SelectionMode)
	{
		case SINGLE_SELECTION:
			...
			{
				_RangeSelectionList.push_back(range);
				produceSelectionChanged(ListSelectionEvent::create(NULL, getSystemTime(), range.StartIndex, range.EndIndex, _ValueIsAdjusting));
			}
			else if (_RangeSelectionList.front() != range)
			{
				...
				_RangeSelectionList.push_back(range);
				updateMinMax();
				produceSelectionChanged(ListSelectionEvent::create(NULL, getSystemTime(), minMax.StartIndex, minMax.EndIndex, _ValueIsAdjusting));
			}
			break;
		case SINGLE_INTERVAL_SELECTION:
			break;
		...
		case MULTIPLE_INTERVAL_SELECTION:
			...
			for(ListItor = _RangeSelectionList.begin() ; ListItor != _RangeSelectionList.end() ; ++ListItor )
			{
				if(index1 >= (*ListItor).StartIndex && index1 <= (*ListItor).EndIndex)
				{
                    ...
                    ShouldProduceSelectionChangeEvent = true;
					TheListSelectionEvent = ListSelectionEvent::create(NULL, getSystemTime(), index1, index1, _ValueIsAdjusting);
				}
			}
			break;
		default:
	...
	switch (_SelectionMode)
	{
		...
		case SINGLE_SELECTION:
			...
			if (!_RangeSelectionList.empty())
			{
				...
				updateMinMax();
				produceSelectionChanged(ListSelectionEvent::create(NULL, getSystemTime(), osgMin(index0,range.StartIndex), osgMax(index0,range.EndIndex), _ValueIsAdjusting));
			}
			else
			{
...
void 	DefaultListSelectionModel::setSelectionMode(UInt32 selectionMode)
{
	...
	if (!_RangeSelectionList.empty())
	{   // only necessary to do anything if it isn't already empty
		const ListSelectionEventUnrecPtr TheEvent = ListSelectionEvent::create(NULL, getSystemTime(), _MinSelectionIndex, _MaxSelectionIndex, _ValueIsAdjusting);
		_RangeSelectionList.clear();
		updateMinMax();
...
void DefaultListSelectionModel::produceSelectionChanged(const ListSelectionEventUnrecPtr e)
{
