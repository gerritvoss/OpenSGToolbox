...
PhysicsJointUnrecPtr buildHingeJointMesh(void);
...
PhysicsJointUnrecPtr buildHingeJointMesh(void)
{
    ...
    for(FCFileType::FCPtrStore::iterator Itor(Containers.begin()) ; Itor != Containers.end() ; ++Itor)
    {
        ...
        {
            TutorialHingeJoint = dynamic_pointer_cast<PhysicsHingeJoint>(*Itor);
        }
    }
...
void buildMotorJointMesh(void)
{
    ...
    //Create AMotor Joint
    PhysicsAMotorJointUnrecPtr TutorialAMotorJoint = PhysicsAMotorJoint::create();
    ...
    PhysicsBallJointUnrecPtr TutorialBallJoint = PhysicsBallJoint::create();
...
void buildBallJointMesh(void)
{
    ...
    TutorialBallJoint->setFirstBody(Box1Body);
    TutorialBallJoint->setSecondBody(Box2Body);
    TutorialBallJoint->setAnchor(Box1Position.subZero() + (Vec3f(Box1Size.x()/2+0.05,0.0,0.0)));
